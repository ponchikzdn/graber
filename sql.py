...
class SQL:
    ...
    def message_id_exists(self, username, message_id: int):
        ...
        with self.connection:
            result = self.cursor.execute("SELECT * FROM DateBase WHERE username=? AND message_id=?", (username, message_id,)).fetchall()
            return bool(len(result))

    def add_message_id(self, username, message_id):
        """Добавление message_id"""
        with self.connection:
            return self.cursor.execute("INSERT INTO DateBase VALUES (?,?)", (username, message_id,))

    def get_last_rowid(self):
        """Получение последнего ROWID"""
        with self.connection:
            return self.cursor.execute("SELECT ROWID, * FROM DateBase LIMIT 1 OFFSET (SELECT COUNT(*) FROM DateBase)-1")

    def get_data_in_table(self, message):
        """Получение записи о посте в таблице"""
        with self.connection:
            return self.cursor.execute(f"SELECT * FROM DateBase WHERE ROWID = {message.text}")
